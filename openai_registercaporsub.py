...
# 强制读取 .env 文件并注入当前环境变量，避免 uv 没挂载的问题
_env_path = OUT_DIR / ".env"
if _env_path.exists():
...
# 强行清除系统或旧 .env 里潜在的错误 HTTP_PROXY，防止 curl_cffi 被毒害把 API 当成直连代理
for k in ["http_proxy", "https_proxy", "HTTP_PROXY", "HTTPS_PROXY", "all_proxy", "ALL_PROXY"]:
    if k in os.environ and ("5010/random" in os.environ[k] or "api" in os.environ[k]):
...
# ========== 临时邮箱提供商：GPTMail + TempMail.lol + LuckMail + Lingpai ==========

class LingpaiMailClient:
    def __init__(self, email_line: str, proxies: Any = None):
        parts = email_line.strip().split("----")
        if len(parts) < 4:
            raise ValueError("灵牌邮箱格式不正确，应为: 邮箱----密码----client_id----refresh_token")
        self.email = parts[0].strip().lower()
        self.password = parts[1]
        self.client_id = parts[2]
        self.refresh_token = "----".join(parts[3:])
        self.proxies = proxies

    def fetch_all_emails(self):
        url = "https://www.lingpaixitong.com/api/mail-all"
        params = {
        ...
        # 使用 py_requests 因为这是独立 API，不需要 curl_cffi 模拟
        import urllib3
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        resp = py_requests.get(url, params=params, proxies=self.proxies, timeout=20, verify=False)
...
class GPTMailClient:
    ...
    def generate_email(self) -> str:
        ...
        resp = self.session.get(f"{self.base_url}/api/generate-email", timeout=30)
    ...
    def list_emails(self, email: str) -> List[Dict[str, Any]]:
        ...
        resp = self.session.get(f"{self.base_url}/api/emails?email={encoded_email}", timeout=30)
        if resp.status_code == 200:
...
class EMail:
    def __init__(self, proxies: Any = None):
        ...
        r = self.s.post("https://api.tempmail.lol/v2/inbox/create", json={}, timeout=30)
        r.raise_for_status()
        data = r.json()
    ...
    def _get_messages(self) -> List[Dict[str, Any]]:
        r = self.s.get(f"https://api.tempmail.lol/v2/inbox?token={self.token}", timeout=30)
        r.raise_for_status()
...
def get_email_and_code_fetcher(
    ...
    if provider not in {"auto", "gptmail", "tempmail", "luckmail", "lingpai"}:
    ...
    def _build_lingpai_bundle():
        lingpai_file = OUT_DIR / "lingpai_emails.txt"
        if not lingpai_file.exists():
            raise RuntimeError(f"Lingpai 邮箱文件不存在: {lingpai_file}")
            
        lines = lingpai_file.read_text(encoding="utf-8").splitlines()
        valid_lines = [l for l in lines if l.strip()]
        if not valid_lines:
            raise RuntimeError(f"Lingpai 邮箱文件已空，请补充 lingpai_emails.txt 以使用该功能！")
            
        email_line = valid_lines.pop(0)
        lingpai_file.write_text("\n".join(valid_lines) + ("\n" if valid_lines else ""), encoding="utf-8")
        
        client = LingpaiMailClient(email_line, proxies)
        email = client.email
        print(f"[+] 提取灵牌邮箱: {email}")
        print("[*] 自带灵牌取回接口，轮询将启动...")
        
        def _extract_all_codes() -> List[str]:
            results: List[str] = []
            try:
                data = client.fetch_all_emails()
                body = json.dumps(data, ensure_ascii=False)
                codes = re.findall(r"(?<!\d)(\d{6})(?!\d)", body)
                results.extend(codes)
                print(f"    [底层轮询] 从灵牌接口全量抓取验证码: 成功提取到 {len(codes)} 个历史码")
            except Exception as e:
                print(f"    [底层轮询] 灵牌接口抓取异常: {e}")
            return results
        ...
        def fetch_code(timeout_sec: int = 180, poll: float = 6.0, exclude_codes: Optional[List[str]] = None) -> str | None:
            ...
            while time.monotonic() - start < timeout_sec:
                ...
                try:
                    ...
                    codes = re.findall(r"(?<!\d)(\d{6})(?!\d)", text_dump)
                    print(f"[otp][lingpai] 轮询 #{attempt}, 提取到验证码数量: {len(codes)}, 目标: {email}")
                    for code in codes:
                        if code not in exclude:
                            return code
                except Exception as e:
                    print(f"[otp][lingpai] 轮询接口异常: {e}")
                time.sleep(poll)
            return None

        # 注意这里仍使用随机生成 OpenAI 密码 _gen_password()，保证强密码
        return email, _gen_password(), fetch_code, _extract_all_codes, "lingpai"

    if provider == "lingpai":
        return _build_lingpai_bundle()
...
def run(
    ...
    try:
        try:
            print(f"[*] 正在检测当前网络代理出口 IP... (当前使用代理配置: {proxy})")
            ip_resp = s.get("https://api.ipify.org?format=json", timeout=10)
            ip_data = ip_resp.json()
            print(f"[调试信息] 当前代理出口 IP: {ip_data.get('ip')}")
        except Exception as e:
            print(f"[调试信息] 获取代理出口 IP 失败 (使用的代理: {proxy}): {e}")
        ...
        auth_page = s.get(oauth.auth_url, timeout=30)
        did = s.cookies.get("oai-did")
        if not did:
            print("[失败] 未能从 Cookie 获取 oai-did")
            print(f"[调试信息] HTTP 状态码: {auth_page.status_code}")
            print(f"[调试信息] 返回头信息: {dict(auth_page.headers)}")
            print(f"[调试信息] 页面内容(前500字): {auth_page.text[:500]}")
            return None
        ...
        signup_res = s.post(
            ...
            data=json.dumps({"username": {"value": email, "kind": "email"}, "screen_hint": "login_or_signup"}),
            timeout=30,
        )
        print(f"[日志] 邮箱提交状态: {signup_res.status_code}")
        print(f"[调试] 步骤3完整响应体: {signup_res.text}")   # ← 加这一行
        print(f"[调试] 步骤3响应头: {dict(signup_res.headers)}")  # ← 加这一行
        if signup_res.status_code != 200:
            print(f"[失败] 邮箱提交失败: {signup_res.text[:200]}")
            return None
        

        # ↓ 新增：跟进 continue_url，把会话推进到密码页状态
        try:
            signup_continue_url = str((signup_res.json() or {}).get("continue_url") or "").strip()
            if signup_continue_url:
                print(f"[*] 跟进注册 continue_url: {signup_continue_url}")
                s.get(signup_continue_url, timeout=30)
            else:
                print("[*] signup_res 无 continue_url，跳过")
        except Exception as e:
            print(f"[*] 跟进 continue_url 异常（忽略）: {e}")
        ...
        # ★ 新增：先访问密码页，让服务端把 session 推进到正确状态
        print("[*] 访问密码页，初始化 session 状态...")
        s.get("https://auth.openai.com/create-account/password", timeout=30)

        # 打印此时的 session cookie，确认有没有 email_verification_mode
        auth_sess_cookie = s.cookies.get("oai-client-auth-session", "")
        print(f"[调试] 步骤3后的 oai-client-auth-session: {auth_sess_cookie[:100]}...")        

        print("[步骤4] 设置账户密码...")
        # ↓ 新增：先拿 sentinel token
        try:
            register_sentinel = _build_sentinel_payload(s, did, "username_password_create")
        except Exception as e:
            print(f"[失败] 获取密码注册 Sentinel 失败: {e}")
            return None
        ...
        pwd_res = s.post(
            ...
            headers={
                ...
                "accept": "application/json",
                "content-type": "application/json",
                "openai-sentinel-token": register_sentinel,  # ← 新增这行
            },
        ...
        if pwd_res.status_code != 200:
            print(f"[失败] 密码设置失败（完整响应）:")
            print(f"  状态码: {pwd_res.status_code}")
            print(f"  响应头: {dict(pwd_res.headers)}")
            print(f"  响应体: {pwd_res.text}")   # ← 去掉截断
            return None
        ...
        for login_attempt in range(3):
            try:
                ...
                s2.get(oauth2.auth_url, timeout=30)
                ...
                if lc.status_code != 200:
                    ...
                    continue
                s2.get(str((lc.json() or {}).get("continue_url") or ""), timeout=30)
                ...
                r = s2.get(str(sel_data["continue_url"]), allow_redirects=False, timeout=30)
                cbk = None
                for i in range(20):
                    ...
                    if r.status_code not in (301, 302, 303) or not loc:
                        break
                    r = s2.get(loc, allow_redirects=False, timeout=30)
        ...
        print("[兜底保护] 登录补全流程 3 次均未完成，但账户确实已经创建成功了！将强制保存账号密码...")
        # 即使没拿到 token_json，也组装一个空字典壳子保住账号
...
def main():
    ...
    parser.add_argument("--proxy-pool", default=os.getenv("PROXY_POOL"), help="代理池 API 获取接口 (返回 IP:PORT 格式，每次自动拉取)")
    parser.add_argument("--mail-provider", choices=["auto", "gptmail", "tempmail", "luckmail", "lingpai"], default="auto", help="临时邮箱提供商：auto/luckmail/gptmail/tempmail/lingpai")
    ...
    while True:
        ...
        # ======== 极简强势代理获取模块 ========
        current_proxy = getattr(args, "proxy", None) or os.environ.get("PROXY")
        proxy_pool = getattr(args, "proxy_pool", None) or os.environ.get("PROXY_POOL")
        if isinstance(proxy_pool, str):
            proxy_pool = proxy_pool.strip()

        if current_proxy:
            current_proxy = current_proxy.strip()
            print(f"[*] 检测到静态代理配置: {current_proxy}")
        elif proxy_pool:
            print(f"[*] 正在请求代理池: {proxy_pool}")
            try:
                resp = py_requests.get(proxy_pool, timeout=10)
                raw_ip = resp.text.strip()
                if resp.status_code == 200 and raw_ip:
                    p = raw_ip if raw_ip.startswith("http") else f"http://{raw_ip}"
                    print(f"[*] 从代理池提取到 IP，立即进入前置测活: {p}")
                    test_s = requests.Session(proxies={"http": p, "https": p}, impersonate="chrome")
                    test_s.get("https://api.ipify.org", timeout=10)
                    print("[成功] 节点测活通过，支持 HTTPS！")
                    current_proxy = p
                else:
                    print(f"[-] 代理池接口异常 (HTTP {resp.status_code}): {raw_ip}")
                    current_proxy = None
                    time.sleep(2)
                    continue
            except Exception as e:
                print(f"[-] 代理拉取或测活崩溃了: {e}")
                current_proxy = None
                time.sleep(2)
                continue
        else:
            print(f"[!] 没扫到任何代理配置! PROXY 和 PROXY_POOL 环境变量全为空或者解析失败！")

        if not current_proxy:
            print("[-] 严厉阻断: 没有任何有效代理 (current_proxy=None)! 拒绝进入直连注册流程，等 2 秒后重新开始...")
            time.sleep(2)
            continue
        # ========================================
