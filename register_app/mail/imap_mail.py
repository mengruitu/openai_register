...
1. 新浪等传统 IMAP 邮箱，从项目根目录 `emails.txt` 读取，格式为：
       邮箱地址----授权码
   也兼容扩展格式：
       邮箱地址----授权码----IMAP服务器----端口

2. 微软 Hotmail / Outlook 邮箱，从项目根目录 `ms_emails.txt` 读取，格式为：
       邮箱地址----密码----client_id----refresh_token
   也兼容扩展格式：
       邮箱地址----密码----client_id----refresh_token----IMAP服务器----端口
...
DEFAULT_MS_IMAP_PORT = 993
DEFAULT_MS_IMAP_SSL = True
DEFAULT_MS_EMAILS_FILE = os.path.join(_SCRIPT_DIR, "ms_emails.txt")
...
_ms_imap_account_lock = threading.Lock()
...
def _load_emails_file(filepath: str = DEFAULT_EMAILS_FILE) -> List[ImapAccount]:
    """从 emails.txt 文件加载新浪等传统 IMAP 邮箱账号列表。"""
    path = str(filepath or "").strip()
...
def _load_ms_emails_file(filepath: str = DEFAULT_MS_EMAILS_FILE) -> List[MicrosoftImapAccount]:
    ...
    accounts: List[MicrosoftImapAccount] = []
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line_no, raw_line in enumerate(f, start=1):
                line = raw_line.strip()
        ...
        logger.warning(f"[IMAP_MS] 读取邮箱账号文件失败: {e}")
        return []

    if accounts:
        logger.info(f"[IMAP_MS] 已加载 {len(accounts)} 个微软邮箱账号")
    else:
        logger.warning(f"[IMAP_MS] 邮箱账号文件中没有有效账号: {path}")

    return accounts


def _ensure_accounts_loaded() -> None:
    """确保传统 IMAP 邮箱账号已加载（懒加载）。"""
...
def remove_imap_account(
    ...
    """从 emails.txt 删除一个已处理的传统 IMAP 账号，并重载账号池。"""
    path = str(filepath or "").strip()
        ...
        try:
            ...
            for raw_line in lines:
                ...
                if not removed and line and not line.startswith("#"):
                    ...
                    if len(parts) >= 2:
                        candidate_email = parts[0].strip()
                        candidate_password = parts[1].strip()
                        if candidate_email == email_value and candidate_password == password_value:
                            removed = True
                            continue
                new_lines.append(raw_line)
...
def select_imap_ms_account() -> Optional[MicrosoftImapAccount]:
    ...
    with _ms_imap_account_lock:
        if not _ms_imap_accounts:
            return None
        index = _ms_imap_account_index % len(_ms_imap_accounts)
...
def _logout_quietly(conn: Optional[imaplib.IMAP4]) -> None:
    if conn is None:
        return
    try:
        conn.logout()
...
def _exchange_microsoft_access_token(account: MicrosoftImapAccount) -> str:
    """使用 refresh token 向微软换取 IMAP access token。"""
    ...
    try:
        ...
        try:
            ...
            detail = str(exc)
        logger.warning(
            f"[IMAP_MS] token 交换失败 ({account.email}): HTTP {exc.code} {detail[:300]}"
        )
        return ""
    except Exception as exc:
        logger.warning(f"[IMAP_MS] token 交换异常 ({account.email}): {exc}")
        return ""
    ...
    logger.warning(
        f"[IMAP_MS] token 交换失败 ({account.email}): {error_code or 'unknown_error'} "
        f"{error_description[:300]}"
    )
    return ""


def _connect_imap_ms(account: MicrosoftImapAccount) -> Optional[imaplib.IMAP4]:
    """建立微软 IMAP 连接并通过 XOAUTH2 登录。"""
    access_token = _exchange_microsoft_access_token(account)
    if not access_token:
        return None
...
def _find_junk_folder(conn: imaplib.IMAP4) -> Optional[str]:
    """尝试找到垃圾箱文件夹名称。"""
    try:
...
def create_imap_mailbox(
    ...
    if not conn:
        logger.error(f"[线程 {thread_id}] [错误] IMAP 邮箱 {account.email} 连接失败")
        return None
...
def create_imap_ms_mailbox(
    ...
    if not conn:
        logger.error(f"[线程 {thread_id}] [错误] 微软 IMAP 邮箱 {account.email} 连接失败")
        return None

    _logout_quietly(conn)

    logger.info(
        f"[线程 {thread_id}] [信息] 使用微软 IMAP 邮箱: {account.email} "
        f"(服务器: {account.imap_host}:{account.imap_port})"
    )
    ...
    return TempMailbox(
        ...
        config_name=f"imap_ms:{account.imap_host}",
        imap_port=account.imap_port,
...
def list_imap_message_ids(
    *,
    ...
    """获取传统 IMAP 邮箱中当前已有的消息 ID 快照（收件箱 + 垃圾箱）。"""
    _ = proxies
...
def list_imap_ms_message_ids(
    *,
    ...
    """获取微软 IMAP 邮箱中当前已有的消息 ID 快照（收件箱 + 垃圾箱）。"""
    _ = proxies, password
    ...
    try:
        ...
        logger.debug(f"[IMAP_MS] 获取消息快照失败 ({email_addr}): {exc}")
        return set()
    finally:
        _logout_quietly(conn)
...
def poll_imap_ms_oai_code(
    *,
    ...
    _log_waiting_code_start(thread_id, email_addr)
    reset_mailbox_wait_diagnostics("imap_ms", email_addr)

    max_polls = 40
    poll_interval = 5

    for _ in range(max_polls):
        increment_mailbox_wait_poll("imap_ms", email_addr)
        ...
        try:
            ...
            total_scanned = len(seen_ids)
            note_mailbox_messages_scanned("imap_ms", email_addr, total_scanned)

            if code:
                mark_mailbox_wait_matched("imap_ms", email_addr, code=code)
                _log_waiting_code_success(thread_id, code)
                return code

        except Exception as exc:
            logger.debug(f"[IMAP_MS] 轮询异常 ({email_addr}): {exc}")
        finally:
            _logout_quietly(conn)
